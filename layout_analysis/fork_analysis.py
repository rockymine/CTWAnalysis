...
def run(args: object) -> None:
    ...
    tier_patches = [
        mpatches.Patch(facecolor=_TIER_COLORS.get(t, "#888888"), label=t)
        for t in _TIER_ORDER if any(r.size_tier == t for r in results)
    ]

    # ---- panel 1: scatter d_graph[0] vs d_graph[1] -------------------------
    ax = axes[0, 0]
    for r in results:
        col = _TIER_COLORS.get(r.size_tier or "", "#888888")
        ax.scatter(r.d_graph[0], r.d_graph[1], color=col,
                   s=28, alpha=0.65, linewidths=0)
    ...
    ax.legend(handles=tier_patches, fontsize=8, framealpha=0.9,
    ...
    ax.axvline(float(np.median(angles)), color="#333333", lw=1.4, ls="--")
    ax.text(float(np.median(angles)) + 1, ax.get_ylim()[1] * 0.92,
            f"median {np.median(angles):.0f}°",
    ...
    ax.set_title("Fork opening angle",
                 fontsize=10, fontweight="bold", color="#111111")

    # ---- panel 3: arm asymmetry --------------------------------------------
    ax = axes[1, 0]
    asym = [r.arm_asymmetry for r in results]
    ax.hist(asym, bins=25, color="#E8835A", edgecolor="white", linewidth=0.5)
    ax.axvline(float(np.median(asym)), color="#333333", lw=1.4, ls="--")
    ax.text(float(np.median(asym)) + 0.01, ax.get_ylim()[1] * 0.92,
            f"median {np.median(asym):.2f}",
            fontsize=8.5, color="#333333", va="top")
    pct_symmetric = 100 * sum(1 for a in asym if a < 0.20) / len(asym)
    ax.text(0.97, 0.92, f"{pct_symmetric:.0f}% within 20% asymmetry",
            transform=ax.transAxes, fontsize=8.5, ha="right", va="top",
            color="#333333",
            bbox=dict(facecolor="white", edgecolor=SPINE_COLOR,
                      boxstyle="round,pad=0.3", alpha=0.9))
    ax.set_xlabel("|d_left − d_right| / max(d_left, d_right)", fontsize=10,
                  color=LABEL_COLOR)
    ax.set_ylabel("Team-halves", fontsize=10, color=LABEL_COLOR)
    ax.set_title("Arm length asymmetry  (0 = perfectly equal)",
                 fontsize=10, fontweight="bold", color="#111111")

    # ---- panel 4: spawn-to-wool graph distance by size tier ----------------
    ax = axes[1, 1]
    ...
    bp = ax.boxplot(
        [tier_data[t] for t in tiers_present],
    ...
    for patch, tier in zip(bp["boxes"], tiers_present):
        patch.set_facecolor(_TIER_COLORS.get(tier, "#888888"))
