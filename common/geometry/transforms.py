...
Canonical Space
---------------
A D4-normalised integer index space used to compare island shapes independent
of their world-space orientation.  Each island is mapped to canonical space by:

  1. Optionally mirroring the X axis.
  2. Rotating by 0, 90, 180, or 270 degrees (around the origin).
  3. Translating so that the minimum coordinate in each axis equals 0.

All three steps use exact integer arithmetic; no floating-point rounding.
...
The mask is built from canonical integer points with a padding border:

    r = z + padding,   c = x + padding

The ``RasterMask.origin`` field records the canonical-space coordinate of
``mask[0, 0]``::

    origin = (-padding, -padding)   # (x, z) in canonical coords

so that the round-trip canonical ↔ raster is always exact:

    rc_to_canonical(r, c) = (c + origin[0], r + origin[1])
    canonical_to_rc(x, z) = (z - origin[1], x - origin[0])

World ↔ Canonical conversions
------------------------------
:class:`CanonicalTransform` stores the mirror/rotation/translation parameters
and provides:

- :meth:`~CanonicalTransform.to_canonical` — world (x, z) block indices →
  canonical (x, z) block indices
- :meth:`~CanonicalTransform.to_original` — canonical (x, z) → world (x, z)

**Important**: these methods operate on *block indices* (integer coordinates).
They must **not** be applied to polygon vertex coordinates (continuous space),
because the "+1 extent" direction changes under rotation.  Always build
Shapely polygons from world-space block indices; see
:func:`~common.geometry.coordinates.world_blocks_to_shapely`.

Raster → World convenience helpers
------------------------------------
:func:`raster_to_world_path` and :func:`raster_to_world_point` combine the
two-step raster → canonical → world conversion that appears in multiple
